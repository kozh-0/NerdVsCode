import { Button, Modal, Radio, Slider, Tooltip, Typography } from "antd";
import { useState } from "react";
import Login from "./Login";
import declOfNum, { LS_Keys } from "../help";
import { creditRates } from "../help";
import { QuestionCircleTwoTone } from "@ant-design/icons";
const { Text } = Typography;

type LS_RATING = "Высокий" | "Низкий" | "Средний" | null;

export default function Credit({ price }: { price: number }) {
  const rubPrice = price * 100;
  const kek = localStorage.getItem(LS_Keys.rating) as LS_RATING;
  const [modal, setModal] = useState({ isOpen: false, confirmLoading: false });
  const [rate, setRate] = useState(0.1);
  const [year, setYear] = useState(5);
  const [initialFee, setInitialFee] = useState(rubPrice * 0.1);

  const radioOptions = [
    { label: "Потреб кредит 10%", value: 0.1 },
    { label: "Автокредит 8%", value: 0.08, disabled: true },
    { label: "Первая машина 4%", value: 0.04, disabled: true },
  ];
  const months = year * 12;
  const monthsRate = rate / 12;

  const ratioA = +(
    (monthsRate * (1 + monthsRate) ** months) /
    ((1 + monthsRate) ** months - 1)
  ); /* .toFixed(2) */

  const creditBody = (rubPrice - initialFee) * ratioA;

  console.log(rubPrice, initialFee, ratioA, creditRates[kek as keyof typeof creditRates]);
  console.log(creditRates);

  return (
    <div style={{ maxWidth: "600px", margin: "0 auto" }}>
      <h2 style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
        <img
          src="https://yt3.googleusercontent.com/ytc/AOPolaRXAKJt-rFPOjizQl5FOGX8ylGNPpRMhJaaY9EbTQ=s900-c-k-c0x00ffffff-no-rj"
          alt="ПСБ"
          style={{ width: "50px", borderRadius: "7px" }}
        />
        &nbsp;Предложение от ПСБ
      </h2>
      <div className="center" style={{ justifyContent: "space-evenly" }}>
        <Radio.Group
          // buttonStyle="solid"
          style={{ display: "flex", justifyContent: "center" }}
          options={radioOptions}
          onChange={(e) => setRate(e.target.value)}
          value={rate}
          optionType="button"
        />
        <Tooltip
          title="Для просмотра заблокированных продуктов, нажмите кнопку узнать"
          placement="right"
        >
          <QuestionCircleTwoTone style={{ transform: "scale(1.5)" }} />
        </Tooltip>
      </div>
      <div style={{ marginTop: "12px" }}>
        Срок
        <div className="center">
          <Slider
            style={{ width: "84%" }}
            value={year}
            min={1}
            max={10}
            onChange={(val) => setYear(val)}
          />
          {year} {declOfNum(year, ["год", "года", "лет"])}
        </div>
      </div>
      Первоначальный взнос
      <div className="center">
        <Slider
          style={{ width: "80%" }}
          value={initialFee}
          min={rubPrice * 0.1}
          max={rubPrice * 0.6}
          step={10000}
          onChange={(val) => setInitialFee(val)}
        />
        <Tooltip title="Вы не можете внести более 60% от стоимости машины" placement="right">
          {initialFee}₽
        </Tooltip>
      </div>
      <div className="center">
        <p>
          {/* {price * ratioA}₽ сегодня <br /> */}
          {/* 31 649Р потом */}
          <Text keyboard style={{ fontSize: "20px" }}>
            {Math.round(creditBody)}₽
          </Text>{" "}
          в месяц на {year} {declOfNum(year, ["год", "года", "лет"])}
        </p>
        <Button
          size="large"
          type="primary"
          onClick={() => {
            Modal.info({
              // maskClosable: true,
              title: "Расчитать ваш кредит",
              open: modal.isOpen,
              closable: true,
              centered: true,
              content: <Login modal={modal} setModal={setModal} />,
              footer: null,
            });
            setModal((p) => ({ ...p, isOpen: true }));
          }}
        >
          Узнать
        </Button>
      </div>
    </div>
  );
}
